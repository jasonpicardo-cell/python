...
def ytd_src(df, today):
    """Year-to-Date: Jan 1 of current year → last completed week (last Friday).
    H = max high since Jan 1,  L = min low since Jan 1,  C = last Friday close.
    This is what TradingView uses for weekly chart Auto pivot — the range grows
    week by week as new highs/lows are established within the current year."""
    jan1 = today.replace(month=1, day=1)
    # Last completed week end = most recent Friday strictly before current week Monday
    wsm      = today - pd.Timedelta(days=today.dayofweek)   # Monday this week
    last_fri = wsm - pd.Timedelta(days=3)                   # Friday last week
    ytd_data = df[(df["Date"] >= jan1) & (df["Date"] <= last_fri)]
    if not len(ytd_data):
        ytd_data = df[df["Date"] >= jan1]                   # fallback: include this week
    if not len(ytd_data): return None
    # C = close of the last Friday (last completed week end)
    last_fri_data = df[df["Date"] <= last_fri]
    pC = r2(float(last_fri_data["Close"].iloc[-1])) if len(last_fri_data) else r2(float(ytd_data["Close"].iloc[-1]))
    cur = df[df["Date"] >= wsm]
    cO  = r2(cur["Open"].iloc[0]) if len(cur) else pC
    return dict(pH=r2(float(ytd_data["High"].max())), pL=r2(float(ytd_data["Low"].min())),
                pC=pC, pO=r2(float(ytd_data["Open"].iloc[0])),
                cO=cO, dt=str(last_fri_data["Date"].iloc[-1].date()) if len(last_fri_data) else str(ytd_data["Date"].iloc[-1].date()))
...
<div class="tb">
  ...
  <span class="bd bb" id="b-src">YEARLY</span>
...
<div class="ft">
  <span>Formulas: TradingView official · All timeframes resampled from daily CSV data · RS = percentile rank of 12M return</span>
...
const TF_SRC = {
  ...
  w:   '<b>Weekly</b> · source = prev Mon–Fri week, resampled from daily CSV (H=max high, L=min low, C=Friday close)',
  m:   '<b>Monthly</b> · source = prev calendar month, resampled from daily CSV (H=max high, L=min low, C=last day close)',
  q:   '<b>Quarterly</b> · source = prev complete quarter, resampled from daily CSV · Q1=Jan–Mar, Q2=Apr–Jun, Q3=Jul–Sep, Q4=Oct–Dec',
  ytd: '<b>YTD</b> · source = Jan 1 of current year → last Friday, resampled from daily CSV (H=YTD high, L=YTD low, C=last Friday close)',
  y:   '<b>Yearly</b> · source = prev complete calendar year (Jan 1 – Dec 31), resampled from daily CSV (H=annual high, L=annual low, C=last trading day close)',
};
...
const DEFAULTS = {
  type:'fibonacci', lv:'S2', tf:'y', pr:'2',
...
function scan() {
  ...
  document.getElementById('b-src').textContent =
    {d:'DAILY',w:'WEEKLY',m:'MONTHLY',q:'QUARTERLY',ytd:'YTD',y:'YEARLY'}[tf] || tf.toUpperCase();
