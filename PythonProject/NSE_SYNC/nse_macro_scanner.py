...
def fetch_data_macro(fyers_symbol):
    # 1. Extract just the core symbol from the Fyers format
    clean_symbol = fyers_symbol.split(':')[1].split('-')[0]

    # 2. Generate the new file path
    file_path = os.path.join(CACHE_DIR, f"{clean_symbol}.csv")
...
def run_turbo_sync():
    ...
    print(f"📊 Total NSE stocks loaded: {len(all_stocks)}")
